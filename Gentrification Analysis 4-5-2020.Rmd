...
UA_Zillow %>%
    filter(Year!=2010) %>%
  ggplot(aes(x = Year, y = Graduate)) +
  geom_smooth(se=FALSE)+
  #scale_x_continuous(breaks=c(2010:2019)) +
  theme(axis.text.x = element_text(angle=90)) +
  labs(title = "Enrollment and Real Estate, Fayetteville, 2010-19", 
       subtitle = "Zillow Housing Index and UA Enrollment",
       caption = "Graphic by Rob Wells, 4-15-2020",
       x="Year",
       y="Graduate")
```
...
UA_Zillow %>%
    filter(Year!=2010) %>%
  group_by(Year, Graduate, Law) %>% 
  ggplot(aes(x = Year, y = Graduate, fill = Law, color=Law)) +
   geom_smooth(se=FALSE)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_text(aes(label = total_pct_change), hjust = -.5, size = 2) +
  scale_y_continuous(limits=c(0, .45),labels = scales::percent) +
  coord_flip() +
  labs(title = "UA Enrollment, 2010-2019",
       subtitle = " ",
       caption = "Graphic by Rob Wells, 4-15-2020",
       y="Source: UA Annual Report",
       x="")
...
EF_type2 %>%
  ...
  labs(title = "Sentiment in News vs. Opinion: China Economic News", 
       ...
       caption = "Bing sentiment dictionary on 4,046 articles. Source: ProQuest. Graphic by Rob Wells",
       x="Year",
       y="Sentiment score")