...
Action <- SF %>% 
  ...
  arrange(desc(n))
```
# Part 2: Basic Cleaning, Processing, Analysis
...
Question #3: Which day had the most calls? Which day had the least?
```{r}
Days %>% 
  filter(n == 232)
...
- **QUESTION #8**: Chart the total dispositions.  
...
Types2 <- SF %>% count(crime2) %>% 
  ...
  arrange(desc(n))
```
- **Question #9:** Take the top 10 crime categories from Type2  
...
Types2 <- SF %>% count(crime2) %>% 
  ...
  arrange(desc(n))
```
- **Question #10:** Make a chart from your cleaned data  
...
```{r}
ggplot(Weekday_Count1, aes(x = weekday, y = n)) +
  xlab("Weekday") +
